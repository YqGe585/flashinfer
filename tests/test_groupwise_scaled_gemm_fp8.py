...
def test_fp8_blockscale_gemm(m, n, k, scale_major_mode, out_dtype):
    paddle.seed(seed=0)
    tile_size = 128
    a_val = paddle.randn(shape=(m, k), dtype="float32")
    b_val = paddle.randn(shape=(n, k), dtype="float32") / math.sqrt(k)
    ...
    ref_c = einops.einsum(a_dequant, b_dequant, "m k, n k -> m n").to(out_dtype)
    c = gemm_fp8_nt_blockscaled(
        a_fp8, b_fp8, a_scale, b_scale, scale_major_mode, out_dtype=out_dtype
    )
    assert paddle.allclose(x=c, y=ref_c, atol=0.01, rtol=0.01).item(), ""
...
def test_fp8_groupwise_gemm(m, n, k, scale_major_mode, backend):
    if backend == "trtllm":
...
@pytest.mark.parametrize("scale_major_mode", ["MN", "K"])
@pytest.mark.parametrize("out_dtype", ["bfloat16"])
def test_fp8_groupwise_group_gemm(m, n, k, group_size, scale_major_mode, out_dtype):
    paddle.seed(seed=0)
    tile_size = 128
    ...
    assert paddle.allclose(x=out, y=ref_c, atol=0.01, rtol=0.01).item(), ""
...
@pytest.mark.parametrize("out_dtype", ["bfloat16"])
def test_fp8_groupwise_group_deepgemm(m, nk, group_size, out_dtype):
    paddle.seed(seed=0)
    m_per_group = m // group_size
    if m_per_group < 128:
        return
    n, k = nk
    a = paddle.randn(shape=(m, k), dtype="float32")
    b = paddle.randn(shape=(group_size, n, k), dtype="float32")
    m_indptr = paddle.empty(shape=(m,), dtype="int32")
    ...
    ref = paddle.empty(shape=(m, n), dtype=out_dtype)
    ...
    assert paddle.allclose(x=out, y=ref, atol=0.03, rtol=0.03).item(), ""
...
@pytest.mark.parametrize("group_size", [1, 4, 8, 64, 128, 256])
@pytest.mark.parametrize("out_dtype", ["bfloat16"])
def test_fp8_groupwise_batch_deepgemm_masked(m, nk, group_size, out_dtype):
    paddle.seed(seed=0)
    n, k = nk
    a = paddle.randn(shape=(group_size, m, k), dtype="float32")
    b = paddle.randn(shape=(group_size, n, k), dtype="float32")
    masked_m = paddle.randint(low=0, high=m, shape=(group_size,), dtype="int32")
    ...
    a_dequant = dequantize_fp8(a_fp8, a_scale, "K")
    b_dequant = dequantize_fp8(b_fp8, b_scale, "K")
    ref = paddle.einsum("bmk,bnk->bmn", a_dequant, b_dequant).to(out_dtype)
    expected_m = min(int(masked_m.astype(dtype="float32").mean()) + 1, m)
    out = batch_deepgemm_fp8_nt_groupwise(
        a_fp8, b_fp8, a_scale, b_scale, masked_m, expected_m, out_dtype=out_dtype
    )
    for i in range(group_size):
        assert paddle.allclose(
            x=out[i][: masked_m[i]], y=ref[i][: masked_m[i]], atol=0.03, rtol=0.03
        ).item(), ""


if __name__ == "__main__":
    test_fp8_blockscale_gemm(8192, 8192, 8192, "MN", "bfloat16")
    test_fp8_groupwise_gemm(8192, 8192, 8192, "K", "bfloat16")
    test_fp8_groupwise_group_gemm(4, 128, 256, 2, "MN", "bfloat16")
    test_fp8_groupwise_group_deepgemm(256, (128, 512), 4, "bfloat16")
    test_fp8_groupwise_batch_deepgemm_masked(256, (128, 512), 8, "bfloat16")

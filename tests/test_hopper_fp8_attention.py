...
>>>>>>        elif dtype == paddle.float8_e5m2:
    ...
    x_max_val = x.abs().amax(axis=(0, 2)).to(dtype="float32")
    s_out = paddle.clip(x=x_max_val / o_max_val, min=1e-06)
    s_out_broadcast = s_out.view(1, -1, 1)
    q_x_out = paddle.clip(x=x / s_out_broadcast, min=o_min_val, max=o_max_val).to(
        dtype=quant_dtype
    )
    assert not paddle.any(x=paddle.isnan(x=q_x_out))
    assert not paddle.any(x=paddle.isnan(x=s_out))
...
def test_single_prefill(seq_len, num_heads, causal, head_dim, dtype):
    ...
    mse = paddle.mean(
        x=(o_ref.astype(dtype="float32") - o_fp8.astype(dtype="float32")) ** 2
    )
...
def test_block_sparse_attention(
    ...
    indptr = paddle.to_tensor(data=S.indptr).cuda()
    indices = paddle.to_tensor(data=S.indices).cuda()
    nnz = S.nnz
    if mask_inside_block:
        data_mask = (paddle.rand(shape=(nnz, R, C)) > 0.5).to("bool").cuda()
    else:
        data_mask = paddle.ones(shape=(nnz, R, C), dtype="bool")
    q = paddle.randn(shape=(M, num_heads, head_dim), dtype="float16")
    k = paddle.randn(shape=(N, num_heads, head_dim), dtype="float16")
    v = paddle.randn(shape=(N, num_heads, head_dim), dtype="float16")
    ...
    mse = paddle.mean(
        x=(o_ref.astype(dtype="float32") - o.astype(dtype="float32")) ** 2
    )

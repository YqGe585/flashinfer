...
def gen_persistent_batch_attention_modules(
    q_dtypes, kv_dtypes, head_dims, use_logits_soft_cap_options
) -> list[JitSpec]:
    jit_specs: list[JitSpec] = []
    for q_dtype, kv_dtype, head_dim, use_logits_soft_cap in itertools.product(
        q_dtypes, kv_dtypes, head_dims, use_logits_soft_cap_options
    ):
        if q_dtype != kv_dtype:
            if kv_dtype.element_size() > 1:
...
def gen_prefill_attention_modules(
    ...
    for (
        ...
        if q_dtype != kv_dtype:
            if kv_dtype.element_size() > 1:
                continue
        if is_sm90a_supported(device2str("cuda")) and is_fa3_backend_supported(
            pos_encoding_mode,
            use_fp16_qk_reduction,

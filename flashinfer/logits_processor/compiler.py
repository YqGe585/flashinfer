...
class Compiler:
    ...
    def _type_check(self, ops: List[Op]) -> None:
        ...
        if current_type not in [TensorType.LOGITS, TensorType.PROBS]:
            raise CompileError(
                f"First operator ({first_op.__class__.__name__}) cannot accept standard pipeline inputs. Expected LOGITS or PROBS, but operator accepts: {first_op.IN}"
            )
        for i, op in enumerate(ops):
            if current_type != op.IN:
                raise CompileError(
                    f"Type mismatch at operator {i} ({op.__class__.__name__}). Expected input type: {current_type}, but operator accepts: {op.IN}. Previous operator output: {current_type}"
                )
            current_type = op.OUT

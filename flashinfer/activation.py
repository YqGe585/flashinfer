...
def get_act_and_mul_module(act_func_name: str):
    ...
    def _act_and_mul(
        out: paddle.Tensor, input: paddle.Tensor, enable_pdl: Optional[bool] = None
    ) -> None:
...
def _check_shape(input: paddle.Tensor, output: paddle.Tensor) -> None:
    assert input.ndim == output.ndim, f"{input.ndim} != {output.ndim}"
    assert (
        tuple(input.shape)[:-1] == tuple(output.shape)[:-1]
    ), f"{tuple(input.shape)[:-1]} != {tuple(output.shape)[:-1]}"
    assert (
        tuple(input.shape)[-1] == 2 * tuple(output.shape)[-1]
    ), f"{tuple(input.shape)[-1]} != {2 * tuple(output.shape)[-1]}"


def silu_and_mul(
    input: paddle.Tensor, out: paddle.Tensor = None, enable_pdl: Optional[bool] = None
) -> paddle.Tensor:
    ...
    if enable_pdl is None:
        enable_pdl = device_support_pdl(input.place)
    if tuple(input.shape)[-1] * input.dtype.itemsize % 16 != 0:
        raise ValueError("The pointers must be multiple of 16 bytes.")
    if out is not None:
        _check_shape(input, out)
    else:
        out = paddle.empty(
            shape=tuple(input.shape)[:-1] + (tuple(input.shape)[-1] // 2,),
            dtype=input.dtype,
        )

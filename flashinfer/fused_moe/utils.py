...
AuxStreamType = Enum("AuxStreamType", ["Attention", "MoeShared", "MoeChunkingOverlap"])
EventType = Enum(
    "EventType", ["Main", "Attention", "MoeShared", "MoeChunkingOverlap"], start=0
)
...
class Fp4QuantizedTensor:
    ...
    @property
    def shape(self):
        return tuple(self.fp4_tensor.shape)
...
def swizzle_sf(sf: paddle.Tensor, rows: int, cols: int, scaling_vector_size: int = 16):
...
def reswizzle_sf(
    ...
    assert sf.size % (padded_rows * padded_sf_cols) == 0
    num_partitions = sf.size // (padded_rows * padded_sf_cols)
...
def _(sf, rows, cols, scaling_vector_size=16):
    ...
    return paddle.empty(shape=sz, dtype=sf.dtype)

...
from ..utils import (dtype_map, mask_mode_literal, pos_encoding_mode_literal,
                     write_if_different)
...
def gen_customize_batch_prefill_tvm_binding(
    ...
    if backend == "auto":
        ...
        for mask_mode, pos_encoding_mode in itertools.product(
            ...
            dest_path = (
                gen_directory
                / f"batch_prefill_paged_kernel_mask_{mask_mode}_pos_encoding_{pos_encoding_mode}.cu"
            )
            source_paths.append(dest_path)
            ...
            dest_path = (
                gen_directory
                / f"batch_prefill_ragged_kernel_mask_{mask_mode}_pos_encoding_{pos_encoding_mode}.cu"
            )
            source_paths.append(dest_path)
            ...
            write_if_different(dest_path, source)
        for filename in ["batch_prefill.cu", "batch_prefill_jit_tvm_binding.cu"]:
            src_path = jit_env.FLASHINFER_TVM_BINDING_DIR / filename
            dest_path = gen_directory / filename
            source_paths.append(dest_path)
            with open(src_path, "r") as f:
        ...
        for mask_mode, pos_encoding_mode in itertools.product([0, 1], [0, 1]):
            filename = f"batch_prefill_paged_sm90_kernel_mask_{mask_mode}_pos_encoding_{pos_encoding_mode}.cu"
            dest_path = gen_directory / filename
            source_paths.append(dest_path)
            source = paged_kernel_inst_templ.render(
                mask_mode=mask_mode_literal[mask_mode],
            ...
            write_if_different(dest_path, source)
            filename = f"batch_prefill_ragged_sm90_kernel_mask_{mask_mode}_pos_encoding_{pos_encoding_mode}.cu"
            dest_path = gen_directory / filename
            source_paths.append(dest_path)
            source = ragged_kernel_inst_templ.render(
                mask_mode=mask_mode_literal[mask_mode],
...
def gen_customize_batch_decode_tvm_binding(
    ...
    for pos_encoding_mode in [0, 1]:
        ...
        source = kernel_inst_templ.render(
            pos_encoding_mode=pos_encoding_mode_literal[pos_encoding_mode], **kwargs
        )
        write_if_different(dest_path, source)
    for filename in ["batch_decode.cu", "batch_decode_jit_tvm_binding.cu"]:
        src_path = jit_env.FLASHINFER_TVM_BINDING_DIR / filename
        dest_path = gen_directory / filename
        source_paths.append(dest_path)
        with open(src_path, "r") as f:

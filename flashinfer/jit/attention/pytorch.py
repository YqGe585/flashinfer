...
from ..core import (JitSpec, gen_jit_spec, logger, sm90a_nvcc_flags,
                    sm100a_nvcc_flags)
from ..utils import (dtype_map, filename_safe_dtype_map, mask_mode_literal,
                     pos_encoding_mode_literal, write_if_different)
...
def get_single_decode_uri(
    ...
    return f"single_decode_with_kv_cache_dtype_q_{filename_safe_dtype_map[dtype_q]}_dtype_kv_{filename_safe_dtype_map[dtype_kv]}_dtype_o_{filename_safe_dtype_map[dtype_o]}_head_dim_qk_{head_dim_qk}_head_dim_vo_{head_dim_vo}_posenc_{pos_encoding_mode}_use_swa_{use_sliding_window}_use_logits_cap_{use_logits_soft_cap}"
...
def get_batch_decode_uri(
    ...
    return f"batch_decode_with_kv_cache_dtype_q_{filename_safe_dtype_map[dtype_q]}_dtype_kv_{filename_safe_dtype_map[dtype_kv]}_dtype_o_{filename_safe_dtype_map[dtype_o]}_dtype_idx_{filename_safe_dtype_map[dtype_idx]}_head_dim_qk_{head_dim_qk}_head_dim_vo_{head_dim_vo}_posenc_{pos_encoding_mode}_use_swa_{use_sliding_window}_use_logits_cap_{use_logits_soft_cap}"
...
def get_batch_mla_uri(
    ...
    return (
        f"batch_mla_attention_dtype_q_{filename_safe_dtype_map[dtype_q]}_dtype_kv_{filename_safe_dtype_map[dtype_kv]}_dtype_o_{filename_safe_dtype_map[dtype_o]}_dtype_idx_{filename_safe_dtype_map[dtype_idx]}_head_dim_ckv_{head_dim_ckv}_head_dim_kpe_{head_dim_kpe}_profiler_{use_profiler}"
...
def get_batch_decode_mla_uri(
    ...
    return f"batch_decode_mla_with_kv_cache_dtype_q_{filename_safe_dtype_map[dtype_q]}_dtype_kv_{filename_safe_dtype_map[dtype_kv]}_dtype_o_{filename_safe_dtype_map[dtype_o]}_dtype_idx_{filename_safe_dtype_map[dtype_idx]}_head_dim_ckv{head_dim_ckv}_use_swa_{use_sliding_window}_use_logits_cap_{use_logits_soft_cap}_arc_{arc}"
...
def gen_batch_decode_mla_module(
    ...
    cuda_arch_major = paddle.device.cuda.get_device_properties(device="gpu:0").major
    if cuda_arch_major >= 9:
        qo_tile_len = 128
    else:
        qo_tile_len = 64
    if (
    ...
    filenames = []
    if arc == "sm80":
        filenames = ["batch_decode_mla_cute_sm80.cu", "batch_decode_mla_pybind.cu"]
    else:
        filenames = [
...
def get_batch_prefill_attention_sink_uri(
    ...
    return (
        f"batch_prefill_with_attention_sink_kv_cache_dtype_q_{filename_safe_dtype_map[dtype_q]}_dtype_kv_{filename_safe_dtype_map[dtype_kv]}_dtype_o_{filename_safe_dtype_map[dtype_o]}_dtype_idx_{filename_safe_dtype_map[dtype_idx]}_head_dim_qk_{head_dim_qk}_head_dim_vo_{head_dim_vo}_use_swa_{use_sliding_window}_"
        + ("_sm90" if backend == "fa3" else "")
    )
...
def get_batch_attention_uri(
    ...
    return f"batch_attention_with_kv_cache_dtype_q_{filename_safe_dtype_map[dtype_q]}_dtype_kv_{filename_safe_dtype_map[dtype_kv]}_dtype_o_{filename_safe_dtype_map[dtype_o]}_dtype_idx_{filename_safe_dtype_map[dtype_idx]}_head_dim_qk_{head_dim_qk}_head_dim_vo_{head_dim_vo}_posenc_{pos_encoding_mode}_use_logits_soft_cap_{str(use_logits_soft_cap).lower()}_use_profiler_{str(use_profiler).lower()}"
...
def gen_single_decode_module(
    ...
    return gen_customize_single_decode_module(
        ...
        f"DefaultAttention<false, {str(use_sliding_window).lower()}, {str(use_logits_soft_cap).lower()}, {str(pos_encoding_mode == 2).lower()}>",
...
def gen_customize_pod_module(
    ...
    for mask_mode_p in [0, 1, 2, 3]:
        for mask_mode_d in [0, 1, 2, 3]:
            ...
            dest_path = gen_directory / filename
            source_paths.append(dest_path)
            source = kernel_inst_templ.render(**kwargs)
            write_if_different(dest_path, source)
...
def gen_customize_single_decode_module(
    ...
    source = kernel_inst_templ.render(**kwargs)
    write_if_different(dest_path, source)
    for filename in ["single_decode.cu", "single_decode_jit_pybind.cu"]:
        src_path = jit_env.FLASHINFER_CSRC_DIR / filename
        dest_path = gen_directory / filename
        source_paths.append(dest_path)
        with open(src_path, "r") as f:
    ...
    if backend == "auto":
        ...
        for mask_mode in [0, 1, 2, 3]:
            ...
            source = kernel_inst_templ.render(
                mask_mode=mask_mode_literal[mask_mode], **kwargs
            )
            write_if_different(dest_path, source)
        for filename in ["single_prefill.cu", "single_prefill_jit_pybind.cu"]:
            src_path = jit_env.FLASHINFER_CSRC_DIR / filename
            dest_path = gen_directory / filename
            source_paths.append(dest_path)
            with open(src_path, "r") as f:
        ...
        for mask_mode in [0, 1, 2, 3]:
            ...
            write_if_different(dest_path, source)
        for filename in [_file_csrc, "single_prefill_sm90_jit_pybind.cu"]:
            src_path = jit_env.FLASHINFER_CSRC_DIR / filename
            dest_path = gen_directory / filename
            source_paths.append(dest_path)
            with open(src_path, "r") as f:
        ...
        write_if_different(generated_config_path, generated_inc_str)
        return gen_jit_spec(uri, source_paths, extra_cuda_cflags=sm90a_nvcc_flags)
    else:
...
def gen_customize_batch_decode_module(
    ...
    source = kernel_inst_templ.render(**kwargs)
    write_if_different(dest_path, source)
    for filename in ["batch_decode.cu", "batch_decode_jit_pybind.cu"]:
        src_path = jit_env.FLASHINFER_CSRC_DIR / filename
        dest_path = gen_directory / filename
        source_paths.append(dest_path)
        with open(src_path, "r") as f:
...
def gen_customize_batch_prefill_module(
    ...
    if backend == "auto":
        ...
        for mask_mode in [0, 1, 2, 3]:
            ...
            write_if_different(dest_path, source)
        for filename in ["batch_prefill.cu", "batch_prefill_jit_pybind.cu"]:
            src_path = jit_env.FLASHINFER_CSRC_DIR / filename
            dest_path = gen_directory / filename
            source_paths.append(dest_path)
            with open(src_path, "r") as f:
        ...
        for mask_mode in [0, 1, 2, 3]:
            ...
            write_if_different(dest_path, source)
        for filename in [_file_csrc, "batch_prefill_sm90_jit_pybind.cu"]:
            src_path = jit_env.FLASHINFER_CSRC_DIR / filename
            dest_path = gen_directory / filename
            source_paths.append(dest_path)
            with open(src_path, "r") as f:
...
def gen_fmha_cutlass_sm100a_module(
    ...
    return gen_jit_spec(uri, source_paths, extra_cuda_cflags=sm100a_nvcc_flags)
...
def gen_customize_batch_attention_module(
    ...
    for mask_mode in [0, 1, 2, 3]:
        ...
        write_if_different(dest_path, source)
    for filename in ["batch_attention.cu", "batch_attention_jit_pybind.cu"]:
        src_path = jit_env.FLASHINFER_CSRC_DIR / filename
        dest_path = gen_directory / filename
        source_paths.append(dest_path)
        with open(src_path, "r") as f:
...
def cudnn_fmha_gen_module():
    return gen_jit_spec(
        ...
        extra_ldflags=["-lcuda"],
        extra_cuda_cflags=[f'-DCUDNN_SDPA_CUBIN_PATH=\\"{ArtifactPath.CUDNN_SDPA}\\"'],
    )

...
def _sdpa_decode_key_fn(
    ...
    *,
    ...
    return "decode", max_sequence_kv, tuple(tuple(q.shape)), tuple(tuple(k_cache.shape))
    ...
    def _build_decode_graph(
        ...
        *,
        ...
        with cudnn.graph(handle) as (g, _):
            if q.dim() == 3:
                s_qo = 1
                b, h_qo, d_qk = tuple(q.shape)[0], tuple(q.shape)[1], tuple(q.shape)[2]
            elif q.dim() == 4:
                b, h_qo, s_qo, d_qk = (
                    tuple(q.shape)[0],
                    tuple(q.shape)[1],
...
def cudnn_batch_decode_with_kv_cache(
    ...
    *,
    ...
    """
    bs = tuple(q.shape)[0]
    h_qo = tuple(q.shape)[1]
    d_vo = tuple(v_cache.shape)[3]
    if out is None:
        out = paddle.empty(shape=[bs, h_qo, d_vo], dtype=q.dtype)
    if not CUDNN_AVAILABLE:
        ...
        actual_seq_lens_q = paddle.ones(shape=(bs, 1, 1, 1), dtype="int32")
        block_size = tuple(k_cache.shape)[2]

...
_is_cuda = True
if _is_cuda:
    CUDA_CAPABILITY = paddle.device.cuda.get_device_capability()
_is_hip = False
...
def extend_attention_fwd(
    ...
    """
    Lq, Lk, Lv = (
        tuple(q_extend.shape)[-1],
        tuple(k_extend.shape)[-1],
        tuple(v_extend.shape)[-1],
    )
    if Lq == 576:
        BLOCK_DMODEL = 512
        BLOCK_DPE = 64
    ...
    batch_size, head_num = tuple(qo_indptr.shape)[0] - 1, tuple(q_extend.shape)[1]
    kv_group_num = tuple(q_extend.shape)[1] // tuple(k_extend.shape)[1]
    ...
    _fwd_kernel[grid](
        ...
        sm_scale,
        kv_group_num,
        q_extend.get_strides()[0],
        q_extend.get_strides()[1],
        k_extend.get_strides()[0],
        k_extend.get_strides()[1],
        v_extend.get_strides()[0],
        v_extend.get_strides()[1],
        o_extend.get_strides()[0],
        o_extend.get_strides()[1],
...
def bench_extend_attention_sink_triton_sgl(
    ...
    extend_len = seq_len // 4
    total_extend_tokens = batch_size * extend_len
    total_prefix_tokens = batch_size * prefill_len
    q_extend = paddle.randn(
        shape=[total_extend_tokens, head_qo_num, head_dim], dtype=dtype
    )
    k_extend = paddle.randn(
        shape=[total_extend_tokens, head_kv_num, head_dim], dtype=dtype
    )
    v_extend = paddle.randn(
        shape=[total_extend_tokens, head_kv_num, head_dim], dtype=dtype
    )
    o_extend = paddle.empty_like(x=q_extend)
    k_buffer = paddle.randn(
        shape=[total_prefix_tokens, head_kv_num, head_dim], dtype=dtype
    )
    v_buffer = paddle.randn(
        shape=[total_prefix_tokens, head_kv_num, head_dim], dtype=dtype
    )
    qo_indptr = paddle.arange(
        start=0, end=(batch_size + 1) * extend_len, step=extend_len
    ).to("int32")
    kv_indptr = paddle.arange(
        start=0, end=(batch_size + 1) * prefill_len, step=prefill_len
    ).to("int32")
    kv_indices = paddle.arange(start=0, end=total_prefix_tokens).to("int32")
    ...
    io = (
        q_extend.size * q_extend.element_size()

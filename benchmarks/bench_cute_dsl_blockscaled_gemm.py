...
def bench_one(num_groups, max_m, expected_m_per_group, n, k):
    ...
    tflops = 2 * valid_m * n * k / t / 1000000000000.0
    gb_per_s = (
        (
    ...
    print(
        f" > Perf (num_groups={num_groups!r}, expected_m_per_group={expected_m_per_group:4}, n={n:4}, k={k:4}): {t * 1000000.0:4.0f} us | {tflops:4.0f} TFLOPS | {gb_per_s:4.0f} GB/s"
    )
    metrics = dict(
        num_groups=num_groups,
        ...
        n=n,
        k=k,
        t_us_raw=t * 1000000.0,
        t_us_calibrated=t_calibrated * 1000000.0,
        tflops=tflops,
        gb_per_s=gb_per_s,
    )
...
def create_masked_m(num_groups, expected_m_per_group, max_m):
    ...
    masked_m = paddle.empty(shape=(num_groups,), dtype="int32")
